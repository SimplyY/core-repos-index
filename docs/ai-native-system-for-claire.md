...
- **事实：** 最近 14 天的本地会话元数据显示，Learn-X 与 Invest-X 的核心 Skill 均有高频调用记录，说明这些不是只写在文档中的设计。
...
最近 14 天本地 Codex 会话元数据的受限统计如下。统计只包含 Skill 名、调用时间、耗时与退出状态，不包含对话正文；“调用”也不等于一次完整业务闭环。
...
最近 14 天元数据中，`asset-report`、`invest-event-watch`、`invest-review` 分别有 77、41、36 次调用记录，活跃于 5 天，未记录到失败。
